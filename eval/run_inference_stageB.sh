...
CKPT_DIR="${CKPT_DIR:-checkpoint/stageB}"          # your trained checkpoint directory
DATA_DIR="${DATA_DIR:-dataset_stageB}"             # your dataset folder
OUT_ROOT="${OUT_ROOT:-outputs/inference_stageB}"   # output root
mkdir -p "${OUT_ROOT}"
...
TEST_FILES=(
  ...
  "test_elec_pred_24h.jsonl"
  "test_elec_pred_72h.jsonl"
)
...
for TEST_FILE in "${TEST_FILES[@]}"; do
  ...
  echo "==== Inference: ${TEST_FILE} -> ${OUT_DIR} ===="

  python inference_mm_stageB.py \
