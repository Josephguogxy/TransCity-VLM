...
class ModelArguments:
    decoder_name_or_path: str = field(metadata={"help": "LLM 解码器"})
    encoder_name_or_path: str = field(metadata={"help": "Chunk 文本编码器"})
    ...
    # 这些 open_* / freeze_decoder 会传给 build_smartcity_llm，用来决定哪些模块 requires_grad=True
    ...
    # mm 模块学习率（encoder / vpma(adapter) / image_projector 等，decoder 的 lr 用 --learning_rate）
    ...
    full_model_load_dir: Optional[str] = field(
        default=None,
        metadata={"help": "Stage-A 权重目录（含 *index.json / safetensors shards）"},
    )
...
def _load_and_merge_cached(files: List[str], split_name: str, seed: int, cache_root: str) -> HFDataset:
    """
    默认：rank0 build + save_to_disk，其他 rank 等待 done_flag 后 load_from_disk。
    可用环境变量控制：
...
# =========================
def main():
    # 关掉 datasets 的进度条（save_to_disk / map 的 tqdm）
    try:
