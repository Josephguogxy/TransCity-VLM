...
def _setup_tokenizer_no_modify(name_or_path: str, padding_token: Optional[str]) -> AutoTokenizer:
    """
    Strict mode:
    ...
    if tok.pad_token_id is None:
        ...
        elif "<|pad|>" in tok.get_vocab() and _token_exists(tok, "<|pad|>"):
            tok.pad_token = "<|pad|>"
        else:
...
def _assert_model_vocab_matches_tokenizer(model, tok: AutoTokenizer) -> None:
    """
    Fail fast if embedding vocab_size != tokenizer vocab_size.
    ...
    if cur != tgt:
        raise ValueError(
            ...
            "Fix: load tokenizer from the exact checkpoint/model directory that matches the decoder weights "
            "(e.g. set decoder_name_or_path to that directory), or use a base model+tokenizer pair with matching vocab."
        )
...
class ModelArguments:
    decoder_name_or_path: str = field(metadata={"help": "LLM decoder (local path or HF id)"})
    encoder_name_or_path: str = field(default="FacebookAI/roberta-large", metadata={"help": "Chunk text encoder"})
    ...
    padding_token: Optional[str] = field(
        default="<|pad|>",
        metadata={"help": "If tokenizer has no pad_token, set pad_token to this existing token (NO new tokens)."},
    )
...
def _load_and_merge_cached(
    ...
    """
    Default behavior: only rank0 builds the dataset and saves it to disk; other ranks wait and load from disk.

    Environment variables:
      SMARTCITY_DATASET_CACHE=0      disable cache
      SMARTCITY_MAP_NUM_PROC=16      num_proc for ensure_smartcity_features
      SMARTCITY_CACHE_ROOT=...       cache root directory
      SMARTCITY_CACHE_WAIT_SEC=86400 wait timeout (seconds)
    """
    assert len(files) > 0, f"No {split_name} files provided."
...
def main():
    ...
    tok = _setup_tokenizer_no_modify(
        model_args.decoder_name_or_path,
        padding_token=model_args.padding_token,
    )
